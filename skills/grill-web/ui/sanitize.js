...
(function (root) {
  ...
  function attrOk(tag, isSvg, name, value) {
    ...
    if (isSvg) return svgValueOk(value);
    return (ATTRS[tag] ?? []).includes(name);
  }

  // svg 프레젠테이션 속성값. CSS 값이라 이스케이프(u\72l)나 주석(u/**/rl)으로 url(을 숨길 수 있다.
  // 역슬래시와 주석은 통째로 버리고, 괄호는 아는 함수만, url()은 #로컬 참조만 허용한다.
  const SVG_FUNCS = new Set(["rotate", "translate", "scale", "matrix", "skewx", "skewy", "rgb", "rgba", "hsl", "hsla", "url"]);
  function svgValueOk(value) {
    if (/[\\]|\/\*/.test(value)) return false;
    for (const m of value.matchAll(/([a-z-]*)\s*\(/gi)) {
      const fn = m[1].toLowerCase();
      if (!SVG_FUNCS.has(fn)) return false;
      if (fn === "url" && !/url\s*\(\s*['"]?#/i.test(value.slice(m.index))) return false;
    }
    return true;
  }
